...
- "Start Free Trial" links to `https://answersnap-backend.fly.dev/checkout` (`backend/main.py`), which creates a Stripe Checkout Session (with 7-day free trial) via the Stripe REST API and 303-redirects to checkout.stripe.com.
- Both `answersnap-backend.fly.dev` and `cheatly-backend.fly.dev` resolve to the same Fly app.
- Backend env: `STRIPE_SECRET_KEY`, `STRIPE_PRICE_ID`, `LANDING_URL` (Fly secrets on app `cheatly-backend`).

## Quick verification (shell)
```
curl -s -o /dev/null -w '%{http_code} %{redirect_url}\n' https://answersnap-backend.fly.dev/checkout
```
Expect `303 https://checkout.stripe.com/c/pay/cs_live_...`. A 500 with "Stripe not configured" means Fly secrets are missing.
...
- All landing page CTAs now say "Get Started" and link to `onboarding.html` (NOT directly to `/checkout`).
- Flow: Landing page "Get Started" → `onboarding.html` (platform selection) → "Continue to Free Trial" → Stripe Checkout.
...
- "Continue to Free Trial" button links to `https://answersnap-backend.fly.dev/checkout`.
- Progress indicator at top: step 1 (active) → step 1 (checkmark) + step 2 (active) after selection.
- Files: `landing/onboarding.html`, `landing/onboarding.css`, `landing/onboarding.js`.

## Testing onboarding flow checklist
1. Verify all 3 landing page CTAs (hero, pricing, sticky bar) say "Get Started" and link to `onboarding.html`
...
- The landing page `#pricing` section shows: "50% Off — Limited Time" urgency badge (gold/yellow, with pulse animation), "$50" strikethrough anchor price before "$25/month", trial messaging ("Try free for 7 days — cancel anytime before your trial ends"), "Get Started" CTA, and "No charge for 7 days" trust line.
- The hero section has a "100% Accuracy" green pill badge between the heading and subtitle, plus the "Get Started" CTA linking to `onboarding.html`.
- A sticky bottom CTA bar (position:fixed, z-index:99) with "Start Free Trial — 7 Days Free" text and a purple "Get Started" button is always visible at the bottom of the viewport. It persists through full page scroll. The footer has extra bottom padding (80px) so it's not hidden behind the bar.
...
- The backend `cancel_url`/`success_url` point at the PROD `LANDING_URL` (https://cheatly.xyz), so cancelling checkout from a Vercel preview returns to prod, not the preview. Expected behavior.
...
5. Sticky CTA button links to `onboarding.html`
...
- Custom events tracked:
  - `click_get_started` with `cta_location` parameter (`hero`, `pricing`, `sticky_bar`) — fired on all "Get Started" CTA clicks on index.html.
...
2. Click hero "Get Started" → verify `en=click_get_started` with `cta_location=hero`
3. Click pricing "Get Started" → verify `en=click_get_started` with `cta_location=pricing`
4. Click sticky bar "Get Started" → verify `en=click_get_started` with `cta_location=sticky_bar`
...
- Deploy: `export PATH="$HOME/.fly/bin:$PATH"; export FLY_API_TOKEN=...; cd backend && flyctl deploy --app cheatly-backend --remote-only`. flyctl may need installing (`curl -L https://fly.io/install.sh | sh`).
...
- Old price: price_1ThMQzACGCy0Kx8oxbHsAZbK ($9.99/week, no longer used but not archived).
...
- New Fly secrets: `SESSION_SECRET`, `STRIPE_WEBHOOK_SECRET` (webhook endpoint we_1ThQw3ACGCy0Kx8o6QF9yiUQ in Stripe).
